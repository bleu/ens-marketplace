...
The reference frontend for the PoC demo flows (Slice 1 and Slice 2). Forkable, never the product — same framing the RFP itself expects: frontend is not a differentiator, the contracts are.
...
| `OPENSEA_API_KEY` | Real OpenSea listings and the Seaport buy flow | The OpenSea source reports itself as not configured; Grails still works |
...
- **ENSv1 · Grails · OpenSea** (mainnet) — real ENS names, read-only ownership, real active listings, a real Seaport buy flow spending real ETH. The default, and the only one that needs no wallet.
...
- **ENSv2 alpha** (Sepolia) — ENS Labs' own real ENSv2 alpha contracts, not our mock. Real commit-reveal registration paid in a real ERC-20. Pre-audit and unofficial; the addresses can change without notice.
...
All four specs are read-only and stub their data at the `/api/ensv1/*` boundary with `cy.intercept`, so they need no chain, no backend, and no API keys. There used to be write-flow specs driving real transactions against a local chain; they went with the local chain itself, so the list/buy/rent flows currently have no automated coverage beyond the Foundry tests.

## Scope

Per the product definition this is a PoC, not a product. Live today: Explore and detail pages for both the ENSv2 mock marketplace and real ENSv1 data, listing and buying, subname leasing, and real ENSv2 alpha registration. Offers, categories, valuation, and collection bids are deliberately inert placeholders — visibly labelled "Coming soon" rather than controls that silently do nothing. See `docs/poc-slice-1.md`, `docs/poc-slice-2.md`, and `docs/roadmap.md`.
