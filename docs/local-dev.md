...
The beta runs against a local Anvil chain with a mock ENSv2-shaped registry (see
...
These are hardcoded in `apps/web/lib/contracts.ts`. If you ever deploy in a different
...
chain ID `31337`, RPC `http://127.0.0.1:8545`, if it's not already there — the web app's
...
With Anvil running and seeded (steps 1-2 above) and the web app running (step 4), from
`apps/web`:
